	...
	public void makeHexGrid(Point origin,int size, int radius, int padding){
        ...
        for (int row = 0; row < size; row++) {
            ...
            for (int col = 0; col < cols; col++) {
                ...
                hexGrid[xLbl+half][yLbl+half] = new Buttons(x,y,radius);
        ...
        for(int i = 0; i < size; i++) {
        	for(int j = 0; j < size; j++) {
        		if(hexGrid[i][j]==null) {
        			hexGrid[i][j]=new Buttons(new Point(0,0),radius);
        		}
        	}
        }
        
        for(coord1 = 0; coord1 < size; coord1++) {
        	for(coord2 = 0; coord2 < size; coord2++) {
        		hexGrid[coord1][coord2].addMouseListener(new MouseListener() {
        			@Override
        			public void mouseClicked(MouseEvent e) {
        				if (hexGrid[coord1][coord2].getColoring().equals(Color.black) && hexGrid[coord1][coord2].getHex().contains(e.getPoint())) {
        					hexGrid[coord1][coord2].setColoring(Color.white);
        				}
        				else if (hexGrid[coord1][coord2].getColoring().equals(Color.white) && hexGrid[coord1][coord2].getHex().contains(e.getPoint())) {
        					hexGrid[coord1][coord2].setColoring(Color.black);
        				}
        			}
